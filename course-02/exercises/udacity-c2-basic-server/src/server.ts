...
(async () => {
  ...
  app.get("/cars", (req: Request, res: Response) => {
    ...
    return res
      .status(200)
      .send({ cars: cars.filter((car) => car.make === make) });
  });
  ...
  app.get("/cars/:id", (req: Request, res: Response) => {
    ...
    if (!id) {
      return res.status(400).send(`the car id is required`);
    }
  ...
  app.post("/cars", async (req: Request, res: Response) => {
    ...
    if (!req.body || !make || !type || !model || !cost) {
      return res
        .status(400)
        .send(`make, type, model, focus, cost are required`);
    }
